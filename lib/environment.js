...
module.exports = (function initCouchBaseEnvironment(couchbase) {
	return class CouchBaseEnvironment extends CouchBaseDB(couchbase) {
		 ...
		 * @returns {Model}
		 *
		 * @see [getModel]{CouchBaseEnvironment.getModel}
		 * @see [createModel]{CouchBaseEnvironment~createModel}
		 * @since 2.0.0
		 */
		model( name ) {
		...
		createModel( bucket, name, schema, methods, prototype, metaProperties ) {
			...
			if ( schema ) {
				...
				//noinspection JSUnresolvedVariable
				if ( schema.useDefaults === void 0 ) {
					//noinspection JSUndefinedPropertyAssignment
					schema.useDefaults = true;
				}
