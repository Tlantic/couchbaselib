...
Couch.prototype.get = function(name){
	...
	Instance.update = function(id, data, cb){
		var x = Model.modelKey(Instance.type, id);
		this.super_.update(x, data, Instance.bucket, Instance.schema, cb);
	}

	Instance.updateWithIndexingViews = function(id, data, cb, views){
		var x = Model.modelKey(Instance.type, id);
		this.super_.updateWithIndexingViews(x, data, Instance.bucket, Instance.schema, cb, views);
	}
