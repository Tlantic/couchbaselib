...
Model.findById = function (key, cb) {
    Model.DB.get(Model.bucket, key, function (err, result) {
        if (err)
            cb(err);
...
Model.prototype.save = function (cb) {
    ...
    if (!errors) {
        Model.DB.insert(Model.bucket, internals.createKey(this.data.uId), this.data, function (err, result) {
            if (err)
                cb(err);
...
Model.remove = function (id, cb) {
    Model.DB.remove(Model.bucket, id, function (err, result) {
        if (err)
            cb(err);
