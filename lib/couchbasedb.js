...
class CouchBaseDB {
	...
	indexViews( bucket, designAndViews, callback ) {
		...
		async.map( designAndViews,
			( view, done ) =>this.query( bucket, ViewQuery.from( view.design, view.viewQuery ), { populate: false }, done ),
			( error, result ) => {
				if ( error ) {
	...
	insert( bucket, key, value, options, callback ) {
		...
		async.waterfall( [
			...
			( data, callback ) =>
				options.views ?
					this.indexViews( bucket, settings.views, ( error ) => callback(error, data) )  : callback( void 0, data )
