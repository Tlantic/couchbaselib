 ...
 * to match the expected return value of a MultiCallback {@see CouchBaseDB.MultiCallback}
...
class CouchBaseDB {
	...
	indexViews( bucket, designAndViews, callback ) {
		...
		async.map(designAndViews, ( view, done ) =>
			this.query( bucket, ViewQuery.from( view.design, view.viewQuery ), done), callback);
	...
	insert(bucket, key, value, options, callback ) {
		...
		async.waterfall([
			...
			(bucket, callback) =>
				bucket.insert(key, value, settings, callback),

			(data, callback) =>
				options.views ?
					this.indexViews( bucket, settings.views, callback ) : callback(void 0, data)

		], (err, results) =>
			settings.populate === true ?
				this.get(bucket, key, callback) : callback(err, results));
	}
	...
	upsert( bucket, key, value, options, callback ) {
		...
		async.waterfall([
			...
			(bucket, callback) =>
				bucket.upsert(key, value, settings, callback),

			(data, callback) =>
				settings.views ?
					this.indexViews( settings.views, callback ) : callback(void 0, data)
	...
	replace(bucket, key, value, options, callback) {
		...
		async.waterfall([
			...
			(bucket, callback) =>
				bucket.replace(docName, newDoc, settings, callback),
	...
	remove (bucket, key, options, callback) {
		...
		async.waterfall([
			...
			(bucket, callback) =>
				bucket.remove(key, settings, callback),
	...
	query(bucket, query, options, callback) {
		...
		if ( query.constructor === CouchBaseDB.ViewQuery ) {
			...
			if ( options.populate === void 0 ) {
				...
				settings.populate = options.populate || false;
			}
		...
		async.waterfall([
			...
			( bucket, callback ) => bucket.query( query, settings, callback),

			( results, meta, callback ) => settings.populate === true && results.length ?
	...
	counter(bucket, key, value, options, callback) {
		...
		async.waterfall([
			...
			(data, callback) =>
				options.views ?
					this.indexViews( options.views, callback ) : callback(void 0, data)
